    ...
    if (ctx.message.from_id == ctx.message.peer_id) // Проверка на ввода личного сообщения боту
    {
        if (!await User.findOne({ID: ctx.message.from_id}).exec()) // Проверка регистрации
        {
            ...
            await ctx.reply(`🤖 [id${ctx.message.from_id}|${data.response[0].first_name}], приветствуем вас!\n\
                                ...
                                Пиши команды! Надеюсь сработаемся)`,null,Markup.keyboard([
                                    [
                                      Markup.button('Напомнить', 'primary'),
                                      Markup.button('Напоминания', 'primary'),
                                    ],
                                    [
                                      Markup.button('Команды', 'positive'),
                                    ],
    ...
    if (ctx.message.from_id == ctx.message.peer_id) // Проверка на ввода личного сообщения боту
    {
        ...
        if(!args[1]) // Проверка на существование аргумента
            return await ctx.reply('🤖 Хмм.. ты не ввёл номер напоминания..',null,Markup.keyboard([
              ...
              ]),); // сообщение и прекращение функции 
        ...
        if(await Reminder.findOne({UserID: ctx.message.from_id, ID: iNumber}).exec()) // поиск напоминания
        {
            ...
            await ctx.reply(`🤖 Напоминание #${iNumber}, было успешно удалено!`,Markup.keyboard([
                [
                  Markup.button('Напомнить', 'primary'),
                  Markup.button('Напоминания', 'primary'),
                ],
...
bot.event('message_new', async (ctx) => {
    if (ctx.message.from_id == ctx.message.peer_id) // Проверка на ввода личного сообщения боту
    {
        ...
        else if (user.Flag == 2) // Проверка на флаг = 2
        {
            ...
            else // Проверка на ввод даты в ручную
            {
                ...
                if (!splitDate[1]) // Проверка на существование данного аргумента
                    return await ctx.reply('🤖 Упс.. видимо ты не так ввёл месяц, попробуй еще раз!',null,Markup.keyboard([
                        [
                          Markup.button('Отмена', 'negative'),
                        ],
                      ]),);  // Ответное сообщение и прекращение функции

                if (!splitDate[2]) // Проверка на существование данного аргумента
                    return await ctx.reply('🤖 Упс.. видимо ты не так ввёл год, попробуй еще раз!',null,Markup.keyboard([
                        [
                          Markup.button('Отмена', 'negative'),
                        ],
                      ]),);  // Ответное сообщение и прекращение функции
                ...
                if(Number(splitDate[0]) < 1 || Number(splitDate[0]) > 31)
                    return await ctx.reply('🤖 Упс.. видимо ты не так ввёл день, попробуй еще раз!',null,Markup.keyboard([
                        [
                          Markup.button('Отмена', 'negative'),
                        ],
                      ]),);  // Ответное сообщение и прекращение функции
                ...
                if((Number(splitDate[1]) == 1 || Number(splitDate[1]) == 3 || Number(splitDate[1]) == 5 || Number(splitDate[1]) == 7 || Number(splitDate[1]) == 8 || Number(splitDate[1]) == 10 || Number(splitDate[1]) == 12) && (Number(splitDate[0]) > 31 || Number(splitDate[0]) < 1))
                    ...
                    Не может быть в таком месяце больше 31 или меньше 1 дня',null,Markup.keyboard([
                        [
                          Markup.button('Отмена', 'negative'),
                        ],
                      ]),);  // Ответное сообщение и прекращение функции   
                ...
                if(((Number(splitDate[1]) == 4 || Number(splitDate[1]) == 6 || Number(splitDate[1]) == 9 || Number(splitDate[1]) == 11)) && (Number(splitDate[0]) > 30 || Number(splitDate[0]) < 1))
                    return await ctx.reply('🤖 Хм.. у тебя тут ошибка:\n\
                    Не может быть в таком месяце больше 30 или меньше 1 дня',null,Markup.keyboard([
                        [
                          Markup.button('Отмена', 'negative'),
                        ],
                      ]),);  // Ответное сообщение и прекращение функции 
                ...
                if (iYear > Number(splitDate[2])) // Проверка прошедшее время
                    return await ctx.reply('🤖 Хм.. у тебя тут ошибка:\n\
                                 Я не могу работать в прошлом, год ввел неправильно',null,Markup.keyboard([
                                    [
                                      Markup.button('Отмена', 'negative'),
                                    ],
                                  ]),);  // Ответное сообщение и прекращение функции 
                    ...
                    return await ctx.reply('🤖 Хм.. у тебя тут ошибка:\n\
                                 Я не могу работать в прошлом, месяц ввел неправильно',null,Markup.keyboard([
                                    [
                                      Markup.button('Отмена', 'negative'),
                                    ],
                                  ]),);  // Ответное сообщение и прекращение функции 
                    ...
                    return await ctx.reply('🤖 Хм.. у тебя тут ошибка:\n\
                             Я не могу работать в прошлом, день ввел неправильно',null,Markup.keyboard([
                                [
                                  Markup.button('Отмена', 'negative'),
                                ],
                              ]),);  // Ответное сообщение и прекращение функции 
            ...
            {
                if(iHour > Number(args[1])) return await ctx.reply('🤖 Хм.. у тебя тут ошибка:\n\
                Я не могу работать в прошлом, час ввел неправильно',null,Markup.keyboard([
                    [
                      Markup.button('Отмена', 'negative'),
                    ],
                  ]),); 
            ...
            else
            {
                ...
                if (Number(splitTime[0]) < 0 || Number(splitTime[0]) > 23)
                    return await ctx.reply('🤖 Упс.. видимо ты не так ввёл часы, попробуй еще раз!',null,Markup.keyboard([
                        [
                          Markup.button('Отмена', 'negative'),
                        ],
                      ]),);  // Ответное сообщение и прекращение функции
                ...
                if (Number(splitTime[1]) < 0 || Number(splitTime[1]) > 60)
                    return await ctx.reply('🤖 Упс.. видимо ты не так ввёл минуты, попробуй еще раз!',null,Markup.keyboard([
                        [
                          Markup.button('Отмена', 'negative'),
                        ],
                      ]),);  // Ответное сообщение и прекращение функции
                ...
                if(Number(iYear) == Number(splitDate[2]) && Number(iMonth) == Number(splitDate[1]) && Number(iDay)== Number(splitDate[1]) && Number(iHour) > Number(splitTime[0])) // Проверка прошедшее время
                    ...
                    return await ctx.reply('🤖 Хм.. у тебя тут ошибка:\n\
                    Я не могу работать в прошлом, минуты ввел неправильно',null,Markup.keyboard([
                        [
                          Markup.button('Отмена', 'negative'),
                        ],
                      ]),);  // Ответное сообщение и прекращение функции 
        ...
        else if(user.Flag == 3)
        {
            ...
            Текст: ${user.Text}\n\
            С повторением: ${messageRepeat}`,Markup.keyboard([
                [
                  Markup.button('Напомнить', 'primary'),
                  Markup.button('Напоминания', 'primary'),
                ],
